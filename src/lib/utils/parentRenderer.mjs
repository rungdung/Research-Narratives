...
export async function loadData(files, supabase) {
	let currentIndex = files.accepted.length - 1;
	let file = files.accepted[currentIndex];
	let fileLocalUrl = URL.createObjectURL(file);
	let fileDBUrl = await loadToDB(file, file.name, supabase);

	const fileName = file.name.split('.')[0];
	const fileType = file.name.split('.')[1];
	renderData(fileName, fileType, fileLocalUrl, fileDBUrl, null, false, supabase);
}
...
export async function renderData(
	...
	try {
		...
		if (fileType.toLowerCase() === 'geojson' || fileType.toLowerCase() === 'spatial') {
