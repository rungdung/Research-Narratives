<script>
	...
	import { Select, Label, Button, Dropdown, Search, NumberInput } from 'flowbite-svelte';
	...
	async function setChloropleth() {
		...
		let binMain = new BinGuru([...selectedAttribute.numericValues], selectedBinCount, null, 1);
		// Bin by method
		...
		let colorExpression = ['interpolate', ['linear'], ['get', selectedAttribute.name]];
...
{#if selectedAttribute && selectedAttribute.dataType === 'numeric'}
	...
	<Select id="method-select" size="sm" class="p-0 px-1 my-1" bind:value={selectedMethod}>
		{#each methods as method}
			<option value={method.name}>{method.name}</option>
		{/each}
	</Select>
	<Label for="value-select">Bin size</Label>
	<NumberInput id="value-select" size="sm" class="p-0 px-1" bind:value={selectedBinCount} />
