<script>
	...
	import { addNewMarkupNodeCollection } from '$lib/utils/addNewNodes.mjs';
	...
	function addToResearchMap() {
		addNewMarkupNodeCollection(selectedLayer.fileName, selectedLayer.name, filterExpression);

		new Alert({
			target: document.body,
			props: {
				pos: 'right',
				content: 'Filtered content from ' + selectedLayer.fileName + ' added to Research Map'
			}
		});
	}
...
<Select id="layerSelect" size="sm" class="p-0 px-1 my-1" bind:value={selectedLayer}>
...
{#if selectedAttribute && selectedAttribute.dataType === 'string'}
	...
	<Button on:click={search} class="rounded-sm px-1 py-0 mt-2 bg-slate-800" id="search-button"
		>Filter</Button
	>
	<Button on:click={addToResearchMap} class="rounded-sm px-1 py-0 mt-2 bg-slate-800">
...
{#if selectedAttribute && selectedAttribute.dataType === 'numeric'}
	...
	<RangeSlider
		...
		step={10}
	/>
	<Button on:click={searchRange} class="rounded-sm px-1 py-0 mt-2 bg-slate-800" id="search-button"
		>Filter</Button
	>
	<Button on:click={addToResearchMap} class="rounded-sm px-1 py-0 mt-2 bg-slate-800">
		Add to Research Map</Button
